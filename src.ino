...
void loop() 
{
  ...
  if (_settings._DebugLog.length() > _settings._BufferSize )
  {_settings._DebugLog=""; }
...
void initHardware()
{
  ...
  //Serial1.begin(1200); // Debugging towards UART1
  ...
  if (!mqttclient.connected()) {
    if (mqttclient.connect((String("ESP-" +_settings._deviceName)).c_str(), _settings._mqttUser.c_str(), _settings._mqttPassword.c_str() )) {
       ...
       Serial1.println(F("Reconnected to MQTT SERVER"));
        _settings._DebugLog += " <br> Reconnected to MQTT SERVER";

        mqttclient.subscribe((topic + String("/Info")).c_str());
        ...
        digitalWrite(Led_Green, LOW); 
         _settings._DebugLog += " <br> CANT CONNECT TO MQTT";
    ...
    _settings._DebugLog += " <br> Data sent to TO MQTT Server";
...
void callback(char* top, byte* payload, unsigned int length) {
  ...
  _settings._DebugLog += " <br> Callback done";
