...
String ip = WiFi.localIP().toString();
    ...
    //s += F("IP:"+ String(ip) +"<br>");
    s += F("<a href=\"debug\">Online Debug Log (Serial Monitor Alternative) </a><br><br><br>");  
...
void serveDebug(WiFiClient& client, String req)
{
  String s = "";
    appendHttp200(s);
    s += F("<H1> Debug Log </H1>\r\n\r\n");
    s += F("<H4> Current Buffer size :");
    s += String((_settings._DebugLog).length()*100/_settings._BufferSize)     +"% </H4> \r\n\r\n";

    s += String(_settings._DebugLog);
    s+= F("</html> \r\n\r\n");
    s+=F("");
    client.print(s);
  delay(1);
