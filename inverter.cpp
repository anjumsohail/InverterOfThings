...
bool onP003PS()
{
  ...
  _settings._DebugLog += " <br> Processing data from :" +_lastRequestedCommand;
...
bool onMOD()
{
  ...
  Serial1.print(F("'"));
  _settings._DebugLog += " <br> QMOD:" +_commandBuffer;
...
bool onPI()
{
  ...
  Serial1.print(_commandBuffer);
  Serial1.print(F("'"));
  _settings._DebugLog += " <br> QPI :" + _commandBuffer;
...
void onInverterCommand()
{
  if ((_commandBuffer.length() > 3) )
  {
    ...
    _settings._DebugLog += " <br> Command sent :" +_lastRequestedCommand;
    _settings._DebugLog += " <br> Received data :" + _commandBuffer;
...
void serviceInverter()
{
  ...
  if ((_lastRequestedCommand == "") && (_lastReceivedAt.compare(INVERTER_COMMAND_DELAY_MS) > 0) && (!_allMessagesUpdated))
  {
    if (_nextCommandNeeded == "") {
      if (inverterType !=2 )  _nextCommandNeeded = "P003GS"; //IF MPI we start with that order
    ...
    _settings._DebugLog += " <br> Command sent :" +_lastRequestedCommand + " For Inverter Type : " + inverterType;
